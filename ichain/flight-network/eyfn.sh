...
function printHelp () {
  ...
  echo
  echo "	eyfn.sh generate -c channelflight"
  echo "	eyfn.sh up -c channelflight -s couchdb"
  echo "	eyfn.sh up -l node"
  echo "	eyfn.sh down -c channelflight"
  echo
...
function networkUp () {
  ...
  if [ ! -d "heathrowairport-artifacts/crypto-config" ]; then
    generateCerts
    generateChannelArtifacts
    createConfigTx
  fi
  # start heathrowairport peers
  ...
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Unable to start Heathrowairport network"
    exit 1
  fi
  echo
  echo "###############################################################"
  echo "############### Have Heathrowairport peers join network ##################"
  echo "###############################################################"
  docker exec Org3cli ./scripts/step2org3.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Unable to have Heathrowairport peers join network"
    exit 1
  fi
  echo
  echo "###############################################################"
  echo "##### Upgrade chaincode to have Heathrowairport peers on the network #####"
  echo "###############################################################"
  docker exec cli ./scripts/step3org3.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Unable to add Heathrowairport peers on network"
    exit 1
  fi
...
# Heathrowairport to the network
function createConfigTx () {
  echo
  echo "###############################################################"
  echo "####### Generate and submit config tx to add Heathrowairport #############"
  echo "###############################################################"
...
# Generates Heathrowairport certs using cryptogen tool
function generateCerts (){
  ...
  echo
  echo "###############################################################"
  echo "##### Generate Heathrowairport certificates using cryptogen tool #########"
  echo "###############################################################"

  (cd heathrowairport-artifacts
   set -x
   cryptogen generate --config=./heathrowairport-crypto.yaml
   res=$?
   set +x
   if [ $res -ne 0 ]; then
...
function generateChannelArtifacts() {
  ...
  echo "##########################################################"
  echo "#########  Generating Heathrowairport config material ###############"
  echo "##########################################################"
  (cd heathrowairport-artifacts
   export FABRIC_CFG_PATH=$PWD
   set -x
   configtxgen -printOrg HeathrowairportMSP > ../channel-artifacts/heathrowairport.json
   res=$?
   set +x
   if [ $res -ne 0 ]; then
     echo "Failed to generate Heathrowairport config material..."
     exit 1
   fi
  )
  cp -r crypto-config/ordererOrganizations heathrowairport-artifacts/crypto-config/
  echo
}
...
#
COMPOSE_FILE_COUCH_ORG3=docker-compose-couch-heathrowairport.yaml
