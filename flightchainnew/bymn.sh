...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"flightchannel\")"
  ...
  echo
  echo "	bymn.sh generate -c flightchannel"
  echo "	bymn.sh up -c flightchannel -s couchdb"
  echo "        bymn.sh up -c flightchannel -s couchdb -i 1.2.x"
  echo "	bymn.sh up -l node"
  echo "	bymn.sh down -c flightchannel"
  echo "        bymn.sh upgrade -c flightchannel"
  echo
...
function replacePrivateKey() {
  ...
  cd crypto-config/peerOrganizations/britishairways.ibs.aero/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# (``peer0.org1.ibs.aero`` & ``peer0.org2.ibs.aero``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for BritishairwaysMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/BirtishairwaysMSPanchors.tx -channelID $CHANNEL_NAME -asOrg BritishairwaysMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for BritishairwaysMSP..."
    exit 1
  fi
...
CLI_DELAY=3
# channel name defaults to "flightchannel"
