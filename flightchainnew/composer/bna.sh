...
configNetwork(){
  ...
  # $2 -Britishairways CA Certificate
  # $3 -Peer0 Address
  # $4 -Peer1 Address
  # $5 -Peer2 Address
  # $6 -Orderer Address
  ...
  cp $CRYPTO_DIR/ordererOrganizations/ibs.aero/orderers/orderer0.ibs.aero/tls/ca.crt $DIR/$WORKING_DIR/orderer0-ca.crt
  cp $CRYPTO_DIR/ordererOrganizations/ibs.aero/orderers/orderer1.ibs.aero/tls/ca.crt $DIR/$WORKING_DIR/orderer1-ca.crt
  cp $CRYPTO_DIR/peerOrganizations/britishairways.ibs.aero/peers/peer0.britishairways.ibs.aero/tls/ca.crt $DIR/$WORKING_DIR/britishairways-ca.crt
  ...
  sed -i -e "s|$KEY2|$(awk 'NF {sub(/\r/, ""); printf "%s\\\\n",$0;}' $DIR/$WORKING_DIR/britishairways-ca.crt)|g" $DIR/multi-network.json
...
createPeerAdmin(){
  ...
  composer card create -p $DIR/multi-network.json -u PeerAdmin -c $DIR/$WORKING_DIR/Admin@britishairways.ibs.aero-cert.pem -k $DIR/$WORKING_DIR/*_sk -r PeerAdmin -r ChannelAdmin -f $DIR/$WORKING_DIR/PeerAdmin@$HLF_NAME.card
