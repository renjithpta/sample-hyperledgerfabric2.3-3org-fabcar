...
EnrollAdmin() {
    ...
    fabric-ca-client enroll -u https://admin:adminpw@localhost:7054 --caname ca.bank.example.com --tls.certfiles ${PWD}/fabric-ca/tls-cert.pem
...
RegisterNewAdmin() {
    ...
    # fabric-ca-client register --id.name admin2 --id.affiliation bank.department1 --id.attrs 'hf.Revoker=true,admin=true:ecert'
    # fabric-ca-client register -d --id.name admin2 --id.affiliation bank.department1 --id.attrs '"hf.Registrar.Roles=peer,client"' --id.attrs hf.Revoker=true
...
EnrollPeer() {
    ...
    fabric-ca-client enroll \
        ...
        --csr.hosts peer0.bank.example.com \
        --tls.certfiles ${PWD}/fabric-ca/tls-cert.pem
}
...
AddNewIdentity() {
    ...
    fabric-ca-client identity add user1 \
        --tls.certfiles ${PWD}/fabric-ca/tls-cert.pem \
        --json '{"secret": "user1pw", "type": "client", "affiliation": "bank", "max_enrollments": 1, "attrs": [{"name": "hf.Revoker", "value": "true"}]}'
...
ModifyIdentity() {
    ...
    fabric-ca-client identity modify user1 --affiliation organization
...
AffiliationOperations() {
    # Add
    fabric-ca-client affiliation add bank.dept1

    # Modify
    fabric-ca-client affiliation modify organization --name user

    # Remove
    fabric-ca-client affiliation remove organization

    # Get All Affiliation
    fabric-ca-client affiliation list

    # Get All with specific affliliation
    fabric-ca-client affiliation list --affiliation organization.dept1
